################################################################################
# L3 RX IDELAY tap sweep (GEM3 → PL), then full 20-frame closeloop on winner.
# argv0 = bitstream (default bitstream_output/system_top_rxdly.bit)
################################################################################
...
proc nsec_dump {{tag ""}} {
    ...
    foreach {name off} {
        ...
        MAC_GOOD 0x28 MAC_FCS 0x2C MAC_BAD 0x64
        MAC_TXG 0x68 RX_ACT 0x6C RGMII_DLY 0x5C
    } {
...
proc gem_tx_one {bytes} {
    ...
    puts [format "WARN: TX timeout BD=0x%08X TXSR=0x%08X" \
...
proc load_tap {tap} {
    global NSEC
    psu_wr [expr {$NSEC + 0x5C}] [expr {($tap & 0x1ff) | 0x10000}]
    after 2
}
...
proc tap_probe {tap n} {
    ...
    after 40
    set act  [psu_rd [expr {$NSEC + 0x6C}]]
    set fcs  [psu_rd [expr {$NSEC + 0x2C}]]
    set bad  [psu_rd [expr {$NSEC + 0x64}]]
    set good [psu_rd [expr {$NSEC + 0x28}]]
    set rx   [psu_rd [expr {$NSEC + 0x10}]]
    return [list $act $fcs $bad $good $rx]
}
...
if {$probe < 0} {
    puts "ERROR: GEM TX not live"; disconnect; exit 1
}

# -------- coarse then fine tap sweep --------
set best_tap 0
set best_rx  0
set best_good 0
set act_seen 0
...
puts "INFO: coarse tap sweep step=16"
for {set tap 0} {$tap < 512} {incr tap 16} {
    lassign [tap_probe $tap 4] act fcs bad good rx
    puts [format "SWEEP tap=%3u ACT=%u FCS=%u BAD=%u GOOD=%u RX=%u" $tap $act $fcs $bad $good $rx]
    if {$act > 0} { set act_seen 1 }
    if {$fcs > 0 || $bad > 0} { set any_fcs 1 }
    if {$rx > $best_rx || ($rx == $best_rx && $good > $best_good)} {
        set best_rx $rx
        set best_good $good
        set best_tap $tap
    }
...
if {!$act_seen} {
    puts "RESULT: RX_ACT=0 all taps — pin has no RGMII RX activity (not a tap problem)"
    nsec_dump "no activity"
    disconnect
    exit 2
}

if {$best_rx == 0} {
    puts "INFO: coarse found no CNT_RX, fine-scan full range around FCS activity"
    set fine_lo 0
    set fine_hi 511
} else {
    set fine_lo [expr {$best_tap - 24}]
    set fine_hi [expr {$best_tap + 24}]
    if {$fine_lo < 0} { set fine_lo 0 }
    if {$fine_hi > 511} { set fine_hi 511 }
    puts [format "INFO: fine sweep tap %u..%u around coarse winner %u (RX=%u)" \
        $fine_lo $fine_hi $best_tap $best_rx]
}

set win_lo -1
set win_hi -1
for {set tap $fine_lo} {$tap <= $fine_hi} {incr tap} {
    lassign [tap_probe $tap 4] act fcs bad good rx
    puts [format "FINE  tap=%3u ACT=%u FCS=%u BAD=%u GOOD=%u RX=%u" $tap $act $fcs $bad $good $rx]
    if {$rx > 0 && $good > 0} {
        if {$win_lo < 0} { set win_lo $tap }
        set win_hi $tap
    }
    if {$rx > $best_rx || ($rx == $best_rx && $good > $best_good)} {
        set best_rx $rx
...
if {$win_lo >= 0} {
    set best_tap [expr {($win_lo + $win_hi) / 2}]
    puts [format "INFO: window tap %u..%u center=%u" $win_lo $win_hi $best_tap]
} else {
    puts [format "WARN: no GOOD+RX window; using best_tap=%u RX=%u GOOD=%u FCS_seen=%d" \
        $best_tap $best_rx $best_good $any_fcs]
}

puts [format "WINNER_TAP=%u" $best_tap]
load_tap $best_tap

# -------- full Dir B 20-frame closeloop --------
pl_arm
set ok 0
...
puts [format "RESULT_B: verdict=%s tap=%u RX=%u FWD=%u DPI=%u MIR=%u TXOK=%u/%u GEM_TXCNT=%u" \
    $verdict_b $best_tap $cnt_rx $cnt_fwd $cnt_dpi $cnt_mir $ok 20 $txcnt]
...
set verdict_a "A_FAIL"
if {$rxcnt > 0} { set verdict_a "A_PASS" }
puts [format "RESULT_A: verdict=%s OCTRX=%u RXCNT=%u FCS=%u" $verdict_a $octrx $rxcnt $fcs]

set pass 0
if {$cnt_rx >= 16 && $cnt_fwd >= 12 && $cnt_dpi >= 3 && $cnt_mir >= 3 && $rxcnt > 0} {
    set pass 1
}
puts [format "PASS: %d  tap=%u  bit=%s" $pass $best_tap [file tail $bit]]
if {$best_rx == 0 && $act_seen} {
    puts "FALLBACK: 0x6C active but no CNT_RX window — need RXC 90-degree sample clock"
}
